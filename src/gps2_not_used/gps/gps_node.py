...
# Using the following library: https://github.com/semuconsulting/pyubx2
...
from pyubx2 import UBXReader, NMEA_PROTOCOL, UBX_PROTOCOL, RTCM3_PROTOCOL, UBXMessage
...
class GPSPublisher(Node):
    ...
    def __init__(self):
        ...
        self.gps = UBXReader(self.sensor_serial, protfilter=UBX_PROTOCOL)
    ...
    def publish(self):
        ...
        raw_data, parsed_data = self.gps.read()

        if not parsed_data: return
        
        self.gps_velocity_data_queue.append((parsed_data.velE, parsed_data.velN))
        
        velE, velN = linear_moving_weighted_average(self.gps_velocity_data_queue)
        
        gps_msg = NavSatFix(longitude=parsed_data.lon, latitude=parsed_data.lat)
        ...
        print(f"Acc: {parsed_data.sAcc}")
        print(f"Sats: {parsed_data.numSV}")
